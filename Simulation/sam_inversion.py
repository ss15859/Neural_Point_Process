...
def set_initial_values(ranges=None):
    if ranges is None:
        ...
        log10_mu_range, log10_k0_range, a_range, log10_c_range, omega_range = ranges
...
def triggering_kernel(metrics, params):
    ...
    time_distance, m = metrics
    theta, mc = params

    log10_mu, log10_k0, a, log10_c, omega = theta
...
def neg_log_likelihood(theta, args):
    mc, n_hat, Pij, source_events, timewindow_length, = args
    ...
    log10_k0, a, log10_c, omega= theta
    k0 = np.power(10, log10_k0)
    c = np.power(10, log10_c)
...
def optimize_parameters(theta_0, ranges, args):
    ...
    mc, n_hat, Pij, source_events, timewindow_length = args
    log10_mu_range, log10_k0_range, a_range, log10_c_range, omega_range = ranges
    log10_mu, log10_k0, a, log10_c, omega = theta_0
    mu = np.power(10, log10_mu)
    k0 = np.power(10, log10_k0)
    c = np.power(10, log10_c)
    ...
    mu_hat = n_hat / (timewindow_length)
    theta_0_without_mu = log10_k0, a, log10_c, omega
...
def invert_etas_params(
    ...
    ranges = log10_mu_range, log10_k0_range, a_range, log10_c_range, omega_range
    ...
    while diff_to_before >= 0.001:
        ...
        print('  maximization\n')
        args = [mc - delta_m / 2, n_hat, Pij, source_events, timewindow_length]
